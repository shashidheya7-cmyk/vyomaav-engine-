...
print("==================================================")
print(" 🌍 PHASE 4C: REAL DENSE MULTI-VIEW WORLD FUSION")
print("==================================================\n")
...
# 1. Write vyomaa/representations/fused_world.py
...
class FusedWorldGeometry:
    fused_points: np.ndarray  # Nx3 (XYZ)
    colors: Optional[np.ndarray] = None  # Nx3 (RGB 0-255 uint8)
    ...
    is_dynamic: Optional[np.ndarray] = None  # N (bool)
...
print("[✓] Created vyomaa/representations/fused_world.py")

# 2. Write vyomaa/fusion/depth_cross_check.py
depth_cross_check_code = '''import numpy as np
from typing import Dict, Any, Tuple

class DepthCrossCheck:
    """
    Cross-checks geometric depth from VGGT against dense relative depth from Depth Anything V2.
    Calculates median scale ratio, Pearson correlation, and disagreement metrics without forcing alignment.
    """
    @staticmethod
    def evaluate(depth_vggt: np.ndarray, depth_da: np.ndarray, threshold: float = 0.25) -> Dict[str, Any]:
        ...
        # Compute median scale alignment
        ...
        # Relative disagreement against scaled reference
...
print("[✓] Created vyomaa/fusion/depth_cross_check.py")

# 3. Write vyomaa/fusion/dense_point_fusion.py (Complete Multi-View World Fusion)
...
from vyomaa.multiview.contracts import ViewSet, CameraEstimate, DenseGeometry
...
class DenseWorldFusionEngine:
    ...
    def fuse_multiview(
        ...
        for v_idx in range(num_views):
            ...
            # 1. Depth Cross-Check with Depth Anything V2 if provided
            ...
            # 3. Unproject 2D pixels to Camera Space (OpenCV convention: X right, Y down, Z forward)
        ...
        for i, pt in enumerate(pts_clean):
            ...
            n_vec = eigenvecs[:, 0]  # Smallest eigenvalue eigenvector
            # Orient normal towards origin/average camera
            if np.dot(n_vec, -pt) < 0:
                n_vec = -n_vec
            normals[i] = n_vec

        # 8. Segregate Dynamic Clusters (Preserved separately, not discarded)
        dynamic_clusters = {}
...
print("[✓] Updated vyomaa/fusion/dense_point_fusion.py")

# 4. Write tests/test_phase4c_fusion.py
...
print("[✓] Created tests/test_phase4c_fusion.py")

# 5. Execute Real End-to-End Multi-View Fusion on GPU (Depth Anything + SAM2 + VGGT)
print("\n==================================================")
print(" 🔬 EXECUTING REAL END-TO-END FUSION BENCHMARK")
print("==================================================")
...
# Step A: VGGT Geometry Estimation
print("[*] Running VGGT multi-view geometry on 5 views...")
...
# Step B: SAM2 Multi-Frame Object Segmentation & Dynamic Masking
print("[*] Running SAM2 multi-frame object perception...")
sam2_segmentations = []
for idx, p in enumerate(image_paths):
    # Seed moving dynamic object
...
# Step C: Depth Anything Dense Depth Representation
...
print(f"[✓] Fused World Geometry PLY exported to: {ply_path}")
